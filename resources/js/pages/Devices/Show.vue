    ...
    <div class="flex flex-1 flex-col gap-6 p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">{{ device.device_name }}</h1>
                    <p class="text-muted-foreground">{{ device.device_code }} • {{ device.location || 'No location' }}</p>
                </div>
            </div>
        ...
        <div class="grid gap-6 md:grid-cols-3">
            <Card class="md:col-span-1">
                <CardHeader>
                    ...
                    <CardDescription>Hardware and network information.</CardDescription>
                </CardHeader>
                <CardContent class="space-y-4">
                    <div class="flex justify-between border-b border-sidebar-border/50 pb-2">
                        <span class="text-sm text-muted-foreground">Status</span>
                        <span :class="['text-sm font-bold uppercase', device.status === 'active' ? 'text-green-600' : 'text-red-600']">
                            {{ device.status }}
                        </span>
                    </div>
                    <div class="flex justify-between border-b border-sidebar-border/50 pb-2">
                        <span class="text-sm text-muted-foreground">Firmware</span>
                        <span class="text-sm font-medium">{{ device.firmware_version || 'Unknown' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-sidebar-border/50 pb-2">
                        <span class="text-sm text-muted-foreground">IP Address</span>
                        <span class="text-sm font-medium font-mono">{{ device.ip_address || 'Not assigned' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-sidebar-border/50 pb-2">
                        <span class="text-sm text-muted-foreground">Last Seen</span>
                        <span class="text-sm font-medium">{{ formatDate(device.last_seen_at) }}</span>
                    </div>
                </CardContent>
            </Card>
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div v-if="latestReading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <MetricCard 
                        label="Voltage" 
                ...
                <div v-else class="rounded-xl border border-dashed p-12 text-center text-muted-foreground italic">
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            <div class="space-y-4">
                <h3 class="text-lg font-semibold flex items-center gap-2">
                    <History class="h-5 w-5 text-blue-500" /> Recent Relay Activity
                </h3>
